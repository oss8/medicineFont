...
gulp.task('fonts', function () {
  if (env === 'development') {
    return gulp.src(entry.fonts)
        .pipe($.plumber(errFun))
        .pipe(gulp.dest(output.dev.fonts));
  }
  return gulp.src(entry.fonts)
      .pipe($.plumber(errFun))
      .pipe(gulp.dest(output.pro.fonts));
});
gulp.task('config', function () {
  if (env === 'development') {
    return gulp.src(entry.config)
        .pipe($.plumber(errFun))
        .pipe(gulp.dest(output.dev.config));
  }
  return gulp.src(entry.config)
      .pipe($.plumber(errFun))
      .pipe(gulp.dest(output.pro.config));
});
...
gulp.task('server', ['init'], function () {
  ...
  gulp.watch(entry.js, ['script'])
      .on('change', browserSync.reload);
  gulp.watch(entry.watchless, ['less']);
...
// 生产环境gulp任务
gulp.task('build', ['init','config'], function() {
